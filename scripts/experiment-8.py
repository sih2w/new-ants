...
class Experiment8Functions:
    ...
    def OnTrainingStepStarted(env: EnvTest, message: Any):
        ...
        for index, agent in enumerate(env["Env"]["Agents"]):
            ...
            env["Rewards"].append(EnvTestFunctions.UpdateAgentWithPriority(env, agent, env["Actions"][index]))
    ...
    def OnTestingStepStarted(env: EnvTest, message: Any):
        for index, agent in enumerate(env["Env"]["Agents"]):
            ...
            EnvTestFunctions.UpdateAgentWithPriority(env, agent, agent["LastAction"])


if __name__ == "__main__":
    file_names = [f"Experiment_8_{index}" for index in range(5)]
    ...
    for index, file_name in enumerate(file_names):
        ...
        if len(episodes) == 0:
            ...
            EventFunctions.Connect(env["Env"]["StepStarted"], lambda *args, **kwargs: Experiment8Functions.OnTrainingStepStarted(env, *args, **kwargs))
            ...
            EventFunctions.Connect(env["Env"]["ProximityDetected"], lambda *args, **kwargs: EnvTestFunctions.OnProximityDetectedFill(env, *args, **kwargs))
        ...
        EventFunctions.Connect(env["Env"]["StepStarted"], lambda *args, **kwargs: Experiment8Functions.OnTestingStepStarted(env, *args, **kwargs))
